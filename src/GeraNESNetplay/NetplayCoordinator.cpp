...
constexpr Netplay::FrameNumber kFreshBootstrapMinimumLagFrames = 90;
...
bool preserveInputSequencesForResync(Netplay::ResyncReason reason,
                                     bool initialSessionSync,
                                     Netplay::FrameNumber targetFrame)
{
    if(reason == Netplay::ResyncReason::ObserverVisibilityRestore) {
        return true;
    }
    if(reason == Netplay::ResyncReason::InitialSessionSync &&
       !initialSessionSync &&
       targetFrame > 0u) {
        return true;
    }
                                                ...
                                                bool preferCurrentFrame)
{
    ...
    const auto priority = [](ResyncReason resyncReason, ParticipantId targetParticipantId) {
        if(resyncReason == ResyncReason::HostStallRecovery) return 5;
        if(resyncReason == ResyncReason::ClientStallRecovery) return 4;
        if(resyncReason == ResyncReason::InitialSessionSync &&
           targetParticipantId != kInvalidParticipantId) return 3;
        if(resyncReason == ResyncReason::ConfirmedDesync) return 2;
        return 1;
    };
...
void NetplayCoordinator::removeParticipant(ParticipantId participantId)
{
    ...
    if(m_remoteInputStallMonitor.pending().has_value() &&
       m_remoteInputStallMonitor.pending()->participantId == participantId) {
        m_remoteInputStallMonitor.reset();
    }
...
bool NetplayCoordinator::sendConfirmedFramesToPeer(NetTransport::PeerHandle peer,
                                                   const std::vector<ConfirmedFrameInputs>& frames,
                                                   Channel channel)
{
    if(peer == NetTransport::kInvalidPeerHandle || frames.empty()) return false;
    ...
    return m_transport.sendReliable(peer, channel, writer.data());
}
           ...
           input.frame < expectedFrame) {
            ...
            if(m_hosting && lateCommittedMismatch && participant->id != m_localParticipantId) {
                ...
                if(participant->lateCommittedDuplicateBurstCount >= 3u) {
                    ...
                    escalation << "Escalating repeated late committed fallback duplicates into targeted recovery for "
                               << participant->displayName
...
bool NetplayCoordinator::handleConfirmedInputFrames(PacketReader& reader)
{
    ...
    std::vector<ConfirmedFrameInputs> frames;
    frames.reserve(data.frameCount);
    ...
    if(m_session.roomState().recoveryInputMode == RecoveryInputMode::ResyncLocked) {
        if(!m_hosting &&
           m_session.roomState().activeResyncId != 0 &&
           data.startFrame > m_session.roomState().resyncTargetFrame &&
           !frames.empty()) {
            if(m_pendingResyncApply.has_value() &&
               m_pendingResyncApply->resyncId == m_session.roomState().activeResyncId) {
                m_pendingResyncApply->runwayFrames.insert(
                    m_pendingResyncApply->runwayFrames.end(),
                    frames.begin(),
                    frames.end()
                );
            } else {
                m_pendingResyncRunwayFrames.insert(
                    m_pendingResyncRunwayFrames.end(),
                    frames.begin(),
                    frames.end()
                );
            }
    ...
    applyConfirmedFrames(frames, false);

    return true;
}

void NetplayCoordinator::applyConfirmedFrames(const std::vector<ConfirmedFrameInputs>& frames,
                                              bool rebuildFutureLocalEntries)
{
    if(frames.empty()) return;

    for(const ConfirmedFrameInputs& frame : frames) {
    ...
    const FrameNumber lastFrame = frames.back().frame;
    ...
    for(const ConfirmedFrameInputs& confirmedFrame : frames) {
        for(PlayerSlot slot : participantAssignments(*localParticipant)) {
            TimelineInputEntry* localEntry =
                m_localInputs.findMutable(confirmedFrame.frame, m_localParticipantId, slot);
            if(localEntry == nullptr &&
               (rebuildFutureLocalEntries || confirmedFrame.frame <= m_localSimulationFrame)) {
                TimelineInputEntry reconstructedEntry{};
                reconstructedEntry.frame = confirmedFrame.frame;
                ...
                localEntry = m_localInputs.findMutable(confirmedFrame.frame, m_localParticipantId, slot);
            }
            if(localEntry != nullptr) {
                localEntry->confirmed = true;
                localEntry->buttonMaskLo = confirmedFrame.buttonMaskLo[slot];
                localEntry->buttonMaskHi = confirmedFrame.buttonMaskHi[slot];
                localEntry->inputFrame = confirmedFrame.inputFrame;
                localEntry->predicted = false;
            }
        }
    }
...
void NetplayCoordinator::noteImplicitRemoteInputStall(ParticipantId participantId, PlayerSlot slot, FrameNumber frame)
{
    ...
    const auto update = m_remoteInputStallMonitor.noteStall(
        participantId,
        slot,
        frame,
...
{
    const auto update = m_remoteInputStallMonitor.clearRecovered(participantId, recoveredThroughFrame);
       ...
       participant.lastContiguousInputFrame < update.recovery.stalledFrame) {
        if(participantNeedsFreshBootstrap(participant)) {
            ...
            queueFreshBootstrapForParticipant(
                participant,
                bootstrapFrame,
                "peer health resumed but usable input did not catch up after synthesized-input recovery"
            );
            return;
        }
        std::ostringstream oss;
...
bool NetplayCoordinator::participantNeedsFreshBootstrap(const ParticipantInfo& participant,
                                                        const ResyncRequestData* request) const
{
    ...
    const bool synthesizedRecoveryActive =
    ...
    const FrameNumber hostReferenceFrame =
        std::max(m_localSimulationFrame, m_session.roomState().lastConfirmedFrame);
    const FrameNumber reportedLag =
        participant.lastReportedCurrentFrame > 0u &&
        hostReferenceFrame > participant.lastReportedCurrentFrame
            ? (hostReferenceFrame - participant.lastReportedCurrentFrame)
            : 0u;
    const FrameNumber inputLag =
        hostReferenceFrame > participant.lastContiguousInputFrame
            ? (hostReferenceFrame - participant.lastContiguousInputFrame)
            : 0u;

    bool requestSaysCannotCatchUp = false;
    if(request != nullptr) {
        const FrameNumber requestLag =
            request->estimatedHostFrame > request->localFrame
                ? (request->estimatedHostFrame - request->localFrame)
                : static_cast<FrameNumber>(request->lagFrames);
        const FrameNumber requestBudget = request->catchupBudgetFrames;
        requestSaysCannotCatchUp =
            request->reason == ResyncReason::ConfirmedDesync &&
            request->source == 1u &&
            requestBudget > 0u &&
            requestLag > requestBudget;
    }

    return (synthesizedRecoveryActive &&
            (reportedLag >= kFreshBootstrapMinimumLagFrames ||
             inputLag >= kFreshBootstrapMinimumLagFrames ||
             requestSaysCannotCatchUp)) ||
           (requestSaysCannotCatchUp &&
            inputLag >= kFreshBootstrapMinimumLagFrames);
}

void NetplayCoordinator::queueFreshBootstrapForParticipant(ParticipantInfo& participant,
                                                           FrameNumber frame,
                                                           const char* detail)
{
    participant.inputSuspended = true;
    ...
    participant.lastLateCommittedDuplicateFrame = 0;
    clearGameplayRecoveryValidation(participant);
    queuePendingHostResync(frame, ResyncReason::InitialSessionSync, participant.id);

    std::ostringstream oss;
    oss << "Scheduling fresh participant bootstrap for " << participant.displayName
        << " frame " << frame;
    if(detail != nullptr && detail[0] != '\0') {
        oss << " reason " << detail;
    }
    oss << " classification=fresh_participant_bootstrap";
    pushLog(oss.str());
}
                                                   ...
                                                   bool preserveConfirmedInputs)
{
    ...
    for(ParticipantInfo& participant : m_session.roomState().participants) {
        const uint32_t latestSequence = latestSequenceForParticipant(participant.id);
...
void NetplayCoordinator::resetRuntimeTimelineStateForSessionStart()
{
    ...
    m_pendingHostResyncFrame.reset();
    m_pendingHostLateJoinResyncParticipant.reset();
    m_pendingResyncRunwayFrames.clear();
    m_remoteInputStallMonitor.reset();
    m_pendingResyncApply.reset();
    m_pendingResyncAcks.clear();
    m_activeResyncExpectedStateCrc32 = 0;
...
bool NetplayCoordinator::handleResyncComplete(PacketReader& reader)
{
    ...
    pending.runwayFrames = std::move(m_pendingResyncRunwayFrames);
...
bool NetplayCoordinator::handleResyncRequest(NetTransport::PeerHandle peer, PacketReader& reader)
{
    ...
    if(m_pendingHostResyncFrame.has_value() &&
       ...
       m_pendingHostResyncFrame->reason == scheduledReason &&
    ...
    oss << "Participant requested authoritative resync: " << participant->displayName
        << " reason " << resyncReasonLabel(data.reason)
        << " frame " << resyncFrame;
    if(scheduledReason != data.reason) {
        oss << " scheduledReason " << resyncReasonLabel(scheduledReason)
            << " targetParticipantId " << static_cast<int>(targetParticipantId);
    }
    if(data.source != 0u) {
                                     ...
                                     ParticipantId targetParticipantId)
{
    ...
    std::vector<ConfirmedFrameInputs> targetedRunwayFrames;
    if(targetedResync) {
    ...
    const bool preserveInputSequences =
        preserveInputSequencesForResync(reason, initialSessionSync, targetFrame);
    if(!targetedResync) {
    ...
    if(targetedResync && m_session.roomState().inputDelayFrames > 0u) {
        const FrameNumber runwayEndFrame =
            targetFrame + static_cast<FrameNumber>(m_session.roomState().inputDelayFrames);
        targetedRunwayFrames.reserve(static_cast<size_t>(runwayEndFrame - targetFrame));
        for(FrameNumber frame = targetFrame + 1u; frame <= runwayEndFrame; ++frame) {
            ConfirmedFrameInputs runwayFrame;
            if(!tryBuildPlaybackFrame(frame, false, runwayFrame, true)) {
                break;
            }
            runwayFrame.predicted = false;
            storeConfirmedFrame(runwayFrame);
            targetedRunwayFrames.push_back(runwayFrame);
        }
        if(!targetedRunwayFrames.empty()) {
            m_session.roomState().lastConfirmedFrame =
                std::max(m_session.roomState().lastConfirmedFrame, targetedRunwayFrames.back().frame);
        }
    }

    if(targetedResync) {
        if(!targetedRunwayFrames.empty()) {
            sendConfirmedFramesToPeer(targetPeer, targetedRunwayFrames, Channel::Control);
        }
...
void NetplayCoordinator::applyResyncRunwayFrames(const std::vector<ConfirmedFrameInputs>& frames)
{
    applyConfirmedFrames(frames, true);
}
