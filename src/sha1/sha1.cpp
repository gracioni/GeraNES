...
void SHA1::update(const char* const data, size_t size)
{
    size_t offset = 0;
    while (offset < size)
    {
        size_t remaining = size - offset;
        size_t bytes_to_read = std::min(remaining, BLOCK_BYTES - buffer.size());
        buffer.append(data + offset, bytes_to_read);
...
std::string SHA1::from_bytes(const char* bytes, size_t size)
{
    SHA1 checksum;
    checksum.update(bytes,size);
    return checksum.final();
}
