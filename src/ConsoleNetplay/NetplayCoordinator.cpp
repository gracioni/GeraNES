...
std::string participantDebugLabel(const ConsoleNetplay::ParticipantInfo& participant)
{
...
std::string controllerAssignmentToast(ConsoleNetplay::PlayerSlot slot, const std::string& participantName)
{
    return ConsoleNetplay::inputAssignmentLabel(slot, ConsoleNetplay::RoomState{}) + " assigned to " + participantName;
}

bool hasReconnectTarget(ConsoleNetplay::NetTransportBackend backend,
                        const ConsoleNetplay::NetTransportOptions& options,
...
std::string describeConnectTarget(ConsoleNetplay::NetTransportBackend backend,
                                  const ConsoleNetplay::NetTransportOptions& options,
                                  const std::string& hostName,
                                  uint16_t port)
{
...
std::string describeHostTarget(ConsoleNetplay::NetTransportBackend backend,
                               const ConsoleNetplay::NetTransportOptions& options,
                               uint16_t port,
                               size_t maxPeers)
{
...
ConsoleNetplay::NetplayInputFrame timelineContributionForSlot(const ConsoleNetplay::TimelineInputEntry& entry)
{
...
{
    return ConsoleNetplay::inputAssignmentLabel(slot, room) + " assigned to " + participantName;
}

std::string participantJoinedToast(const ConsoleNetplay::ParticipantInfo& participant)
{
    return participantLabel(participant) + " joined";
}

std::string participantReconnectedToast(const ConsoleNetplay::ParticipantInfo& participant)
{
...
bool preserveInputSequencesForResync(ConsoleNetplay::ResyncReason reason,
                                     bool initialSessionSync,
                                     ConsoleNetplay::FrameNumber targetFrame)
{
...
ConsoleNetplay::AssignControllerData makeAssignControllerData(const ConsoleNetplay::ParticipantInfo& participant)
{
...
void NetplayCoordinator::setRepeatedInputFrameTransformer(
    std::function<NetplayInputFrame(const NetplayInputFrame&, FrameNumber)> transformer)
{
    m_repeatedInputFrameTransformer = std::move(transformer);
}

NetplayInputFrame NetplayCoordinator::repeatedInputFrameFrom(const NetplayInputFrame& previous,
                                                            FrameNumber targetFrame) const
{
    if(m_repeatedInputFrameTransformer) {
        return m_repeatedInputFrameTransformer(previous, targetFrame);
    }
    return NetplayInputFrame::repeatedFrom(previous, targetFrame);
}
...
bool NetplayCoordinator::handleInputFrame(NetTransport::PeerHandle peer, PacketReader& reader)
{
    ...
    if(participant != nullptr) {
        ...
        if(allowSequenceRebase || allowClientResyncRebase) {
            ...
            std::ostringstream oss;
            oss << "Accepted input rebase from "
                << participant->displayName
                << " frame " << input.frame
                << " expectedFrame " << expectedFrame
                << " seq " << input.sequence
    ...
    for(ParticipantInfo& participant : m_session.roomState().participants) {
        ...
        for(PlayerSlot slot : participantAssignments(participant)) {
            ...
            while(nextFrame <= targetFrame) {
                ...
                synthetic.netplayFrame = repeatedInputFrameFrom(latestConfirmed->netplayFrame, nextFrame);
...
bool NetplayCoordinator::predictRemoteInputFrame(FrameNumber frame, ParticipantId participantId, PlayerSlot slot)
{
    ...
    predicted.netplayFrame = repeatedInputFrameFrom(lastKnown->netplayFrame, frame);
