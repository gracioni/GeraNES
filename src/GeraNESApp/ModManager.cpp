...
void ModManager::renderReplacements(std::vector<uint32_t>& framebuffer, int width, int height, int activeTop, int activeBottom, int scale, GeraNESEmu& emu)
{
    if(!m_active || framebuffer.empty() || width <= 0 || height <= 0 || scale <= 0) {
        return;
    }
    renderBackgroundReplacements(framebuffer, width, height, activeTop, activeBottom, scale, emu);
    ...
    for(const ChrOverride& override : m_chrOverrides) {
        if(override.enabled && !override.assetPath.empty() && conditionsMatch(override.conditions, emu)) {
...
ModManager::ChrRenderSnapshot ModManager::captureChrRenderSnapshot(GeraNESEmu& emu) const
{
    ...
    for(size_t tile = 0; tile < snapshot.tileHashes.size(); ++tile) {
        snapshot.tileHashes[tile] = hashChrTile(ppu, static_cast<int>(tile * 16));
    }
    return snapshot;
}

void ModManager::renderBackgroundReplacements(std::vector<uint32_t>& framebuffer, int width, int height, int activeTop, int activeBottom, int scale, GeraNESEmu& emu)
{
    if(!m_active || framebuffer.empty() || width <= 0 || height <= 0 || m_backgroundReplacements.empty()) {
        return;
    }

    std::vector<BackgroundReplacement*> drawList;
    drawList.reserve(m_backgroundReplacements.size());
    for(BackgroundReplacement& replacement : m_backgroundReplacements) {
        if(replacement.enabled) drawList.push_back(&replacement);
    }
    std::stable_sort(drawList.begin(), drawList.end(), [](const BackgroundReplacement* a, const BackgroundReplacement* b) {
    ...
    for(BackgroundReplacement* replacement : drawList) {
        if(!conditionsMatch(replacement->conditions, emu)) {
            continue;
        }
        if(!patternMatches(*replacement, emu)) {
            continue;
        }

        const DecodedImage* image = decodedImage(replacement->assetPath);
        if(image == nullptr || image->rgba.empty() || image->width <= 0 || image->height <= 0) {
            continue;
        }

        const int dstX = replacement->x * scale;
        const int dstY = replacement->y * scale;
        const int dstWidth = std::max(0, replacement->width) * scale;
        const int dstHeight = std::max(0, replacement->height) * scale;
        const int dstX0 = std::clamp(dstX, 0, width);
        const int dstY0 = std::clamp(std::max(dstY, activeTop), 0, height);
        const int dstX1 = std::clamp(dstX + dstWidth, 0, width);
        const int dstY1 = std::clamp(std::min(dstY + dstHeight, activeBottom), 0, height);
        if(dstX0 >= dstX1 || dstY0 >= dstY1) {
            continue;
        }

        const int cameraX = static_cast<int>(std::lround(readCameraValue(replacement->cameraX, emu) * replacement->parallaxX));
        const int cameraY = static_cast<int>(std::lround(readCameraValue(replacement->cameraY, emu) * replacement->parallaxY));
        const int effectiveScrollX = (replacement->scrollX + cameraX) * scale;
        const int effectiveScrollY = (replacement->scrollY + cameraY) * scale;
        const int alphaScale = std::clamp(static_cast<int>(std::lround(replacement->opacity * 255.0f)), 0, 255);

        const int nesX0 = std::max(0, dstX0 / scale);
        const int nesY0 = std::max(0, dstY0 / scale);
        const int nesX1 = std::min(PPU::SCREEN_WIDTH, (dstX1 + scale - 1) / scale);
        const int nesY1 = std::min(PPU::SCREEN_HEIGHT, (dstY1 + scale - 1) / scale);

        for(int nesY = nesY0; nesY < nesY1; ++nesY) {
            const int blockY0 = std::clamp(nesY * scale, dstY0, dstY1);
            const int blockY1 = std::clamp((nesY + 1) * scale, dstY0, dstY1);
            if(blockY0 >= blockY1) continue;
            for(int nesX = nesX0; nesX < nesX1; ++nesX) {
                const int blockX0 = std::clamp(nesX * scale, dstX0, dstX1);
                const int blockX1 = std::clamp((nesX + 1) * scale, dstX0, dstX1);
                if(blockX0 >= blockX1) continue;

                uint32_t& firstDst = framebuffer[static_cast<size_t>(blockY0) * static_cast<size_t>(width) + static_cast<size_t>(blockX0)];
                const PPU::DebugModBackgroundPixel& capturedPixel = emu.getConsole().ppu().debugModBackgroundPixelAt(nesX, nesY);
                const uint8_t paletteIndex = capturedPixel.valid
                    ? capturedPixel.paletteIndex
                    : readBackgroundPaletteIndexAt(
                        emu.getConsole().ppu(),
                        std::clamp(nesX, 0, PPU::SCREEN_WIDTH - 1),
                        std::clamp(nesY, 0, PPU::SCREEN_HEIGHT - 1)
                    );
                if(!replacementPixelAllowed(*replacement, firstDst, paletteIndex)) {
                    continue;
                }

                for(int outY = blockY0; outY < blockY1; ++outY) {
                    int srcY = replacement->sourceY + (outY - dstY) + effectiveScrollY;
                    if(replacement->repeatY) {
                        srcY = wrapCoord(srcY, image->height);
                    }
                    if(srcY < 0 || srcY >= image->height) continue;
                    for(int outX = blockX0; outX < blockX1; ++outX) {
                        int srcX = replacement->sourceX + (outX - dstX) + effectiveScrollX;
                        if(replacement->repeatX) {
                            srcX = wrapCoord(srcX, image->width);
                        }
                        if(srcX < 0 || srcX >= image->width) continue;
                        uint32_t& dst = framebuffer[static_cast<size_t>(outY) * static_cast<size_t>(width) + static_cast<size_t>(outX)];
                        dst = blendPixel(dst, image->rgba[static_cast<size_t>(srcY) * static_cast<size_t>(image->width) + static_cast<size_t>(srcX)], alphaScale);
                    }
                }
            }
        }
...
void ModManager::renderChrOverrides(std::vector<uint32_t>& framebuffer, int width, int height, int activeTop, int activeBottom, int scale, const ChrRenderSnapshot& snapshot, const std::vector<const ChrOverride*>* activeOverrideFilter)
{
    if(m_chrOverrides.empty() || scale <= 0) return;
    ...
    if(activeOverrides.empty()) return;
    ...
    if(preparedOverrides.empty()) return;
    ...
    auto findOverride = [&](ChrOverride::Target target, int tileIndex, int fullTileIndex, int patternAddress, int currentPatternTable, const std::array<uint8_t, 3>& palette) -> const ChrOverride* {
        ...
        // Mesen's default tile lookup uses the same tile key with palette ignored.
        // It does not broaden the search to alternate tile IDs or whole-CHR rules.
    ...
    auto drawTilePixel = [&](uint32_t baseColor, uint32_t /*fallbackLayerColor*/, const ChrOverride* override, int patternAddress, int offsetX, int offsetY, int subX, int subY, uint8_t /*colorLowBits*/, const std::array<uint8_t, 3>& palette, bool horizontalMirror = false, bool verticalMirror = false, bool preserveSourceAlpha = false) {
        if(override == nullptr) {
            return baseColor;
        }
    ...
    for(int nesY = activeNesY0; nesY < activeNesY1; ++nesY) {
        ...
        for(int nesX = 0; nesX < PPU::SCREEN_WIDTH; ++nesX) {
            ...
            if(pixelIndex >= snapshot.renderedPixels.size()) {
                continue;
            }
            const PPU::DebugModRenderedPixel& pixel = snapshot.renderedPixels[pixelIndex];
            if(!pixel.valid) {
                continue;
            }
            ...
            const std::array<uint8_t, 3> palette = { pixel.palette[0], pixel.palette[1], pixel.palette[2] };
            const int patternAddress = pixel.patternAddress;
            const int fullTileIndex = fullTileIndexFromPatternAddress(patternAddress);
            if(fullTileIndex < 0) {
                continue;
            }
            const ChrOverride::Target target =
                pixel.layer == PPU::DebugModRenderLayer::Sprite
                    ? ChrOverride::Target::Sprite
                    : ChrOverride::Target::Background;
            const ChrOverride* override =
                onlyWholeChrOverrides
                    ? (target == ChrOverride::Target::Sprite ? fastSpriteOverride : fastBackgroundOverride)
                    : findOverride(
                        target,
                        fullTileIndex & 0xFF,
                        fullTileIndex,
            ...
            const uint32_t fallbackColor = snapshot.paletteColors[pixel.paletteIndex & 0x3F];
            ...
            for(int subY = 0; subY < scale; ++subY) {
                ...
                for(int subX = 0; subX < scale; ++subX) {
                    ...
                    uint32_t& color = framebuffer[static_cast<size_t>(outY) * static_cast<size_t>(width) + static_cast<size_t>(outX)];
                    if(override != nullptr) {
                        color = drawTilePixel(
                            color,
                            fallbackColor,
                            ...
                            pixel.colorLowBits,
                            palette,
                            pixel.horizontalMirror,
                            pixel.verticalMirror
                        );
                    }
                }
            }
