    ...
    void updateInputs()
    {
        ...
        for(int i = 0; i < numControllers; i++) {
            ...
            for(int j = 0; j < SDL_CONTROLLER_BUTTON_MAX; j++) {
                ...
                name << "J" << i << "B" << j;
                setInputState(name.str(), SDL_GameControllerGetButton(controller, static_cast<SDL_GameControllerButton>(i)));
            }
            ...
            for(int j = 0; j < SDL_CONTROLLER_AXIS_MAX; j++) {
                ...
                posName << "J" << i << "A" << j << "+";
                bool state = SDL_GameControllerGetAxis(controller, static_cast<SDL_GameControllerAxis>(i)) > ANALOG_TO_DIGITAL_THRESHOLD * 0x7FFF;
                setInputState(posName.str(), state);

                std::stringstream negName;
                negName << "J" << i << "A" << j << "-";
                state = SDL_GameControllerGetAxis(controller, static_cast<SDL_GameControllerAxis>(i)) < ANALOG_TO_DIGITAL_THRESHOLD * -0x7FFF;
