...
public:
    ...
    GERANES_HOT uint8_t readPrg(int addr) override
    {
        ...
        case 0:
            if(addr < 0x4000) return m_cd.readPrg<WindowSize::W16K>(m_PRGBank,addr);
            return m_cd.readPrg<WindowSize::W16K>(m_PRGBank | 1,addr);
            break;

        case 1:
            if(addr < 0x4000) return m_cd.readPrg<WindowSize::W16K>(m_PRGBank,addr);
            return m_cd.readPrg<WindowSize::W16K>(m_PRGBank | 7,addr);
            break;

        case 2:
            return m_cd.readPrg<WindowSize::W8K>((m_PRGBank << 1) | m_b,addr);
            break;

        case 3:
            return m_cd.readPrg<WindowSize::W16K>(m_PRGBank,addr);
            break;
        }
