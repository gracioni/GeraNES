...
class Mapper054 : public BaseMapper
{
private:
    uint8_t m_prgBank = 0;
    uint8_t m_chrBank = 0;
...
public:
    Mapper054(ICartridgeData& cd) : BaseMapper(cd)
    {
    ...
    {
        const uint8_t absolute = static_cast<uint8_t>(addr + 0x8000);
        m_prgBank = absolute & 0x03;
        m_chrBank = absolute & 0x07;
        m_prgBank &= m_prgMask;
        m_chrBank &= m_chrMask;
    }
