...
public:
    ...
    {
        switch(addr>>13) {
        case 0: return m_cd.readPrg<WindowSize::W8K>(m_PRGREG[1],addr);
        case 1: return m_cd.readPrg<WindowSize::W8K>(m_PRGREG[2],addr);
        case 2: return m_cd.readPrg<WindowSize::W8K>(m_PRGREG[3],addr);
    ...
    GERANES_HOT uint8_t readChr(int addr) override
    {
        ...
        return m_cd.readChr<WindowSize::W1K>(m_CHRREG[index],addr);
    }
    ...
    {
        if(m_PRGRAMEnable && m_PRGRAMSelect) writeCHRRAM<WindowSize::W8K>(m_PRGREG[0], addr, data);
    }

    GERANES_HOT uint8_t readSaveRam(int addr) override
    {
        if(m_PRGRAMEnable && m_PRGRAMSelect)
            return readCHRRAM<WindowSize::W8K>(m_PRGREG[0],addr);

        return m_cd.readPrg<WindowSize::W8K>(m_PRGREG[0],addr);
    }
