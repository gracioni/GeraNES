...
class Mapper215 : public Mapper004
{
private:
    uint8_t m_exRegs[3] = {0, 3, 0};

    static constexpr uint8_t LUT_REG[8][8] = {
        { 0, 1, 2, 3, 4, 5, 6, 7 },
        { 0, 2, 6, 1, 7, 3, 4, 5 },
        { 0, 5, 4, 1, 7, 2, 6, 3 },
        { 0, 6, 3, 7, 5, 2, 4, 1 },
    ...
    static constexpr uint8_t LUT_ADDR[8][8] = {
        { 0, 1, 2, 3, 4, 5, 6, 7 },
        { 3, 2, 0, 4, 1, 5, 6, 7 },
        { 0, 1, 2, 3, 4, 5, 6, 7 },
        { 5, 0, 1, 2, 3, 7, 6, 4 },
    ...
    {
        if(m_exRegs[0] & 0x40) {
            return static_cast<uint16_t>((((m_exRegs[1] & 0x0C) << 6) | (page1k & 0x7F) | ((m_exRegs[1] & 0x20) << 2))) & m_chrMask;
        }
        return static_cast<uint16_t>((((m_exRegs[1] & 0x0C) << 6) | page1k)) & m_chrMask;
    }
    ...
    GERANES_INLINE uint8_t mapPrgPage(int slot, uint8_t page8k) const
    {
        ...
        if(m_exRegs[0] & 0x40) {
            mask = 0x0F;
            sbank = static_cast<uint8_t>(m_exRegs[1] & 0x10);
            if(m_exRegs[0] & 0x80) {
                bank = static_cast<uint8_t>(((m_exRegs[1] & 0x03) << 4) | (m_exRegs[0] & 0x07) | (sbank >> 1));
            }
        }
        else {
            mask = 0x1F;
            if(m_exRegs[0] & 0x80) {
                bank = static_cast<uint8_t>(((m_exRegs[1] & 0x03) << 4) | (m_exRegs[0] & 0x0F));
            }
        }
        ...
        if(m_exRegs[0] & 0x80) {
            ...
            if(m_exRegs[0] & 0x20) {
                return static_cast<uint8_t>(bank + slot) & m_prgMask;
            }
            return static_cast<uint8_t>(bank + (slot & 0x01)) & m_prgMask;
        }

        return static_cast<uint8_t>((((m_exRegs[1] & 0x03) << 5) | (page8k & mask) | sbank)) & m_prgMask;
    }

public:
    Mapper215(ICartridgeData& cd) : Mapper004(cd)
    {
    }
    ...
    {
        switch(addr) {
        case 0x5000: m_exRegs[0] = value; break;
        case 0x5001: m_exRegs[1] = value; break;
        case 0x5007: m_exRegs[2] = value; break;
        default: return;
        }
    }
    ...
    GERANES_HOT void writePrg(int addr, uint8_t value) override
    {
        ...
        const uint8_t lutValue = LUT_ADDR[m_exRegs[2] & 0x07][((absolute >> 12) & 0x06) | (absolute & 0x01)];
        const uint16_t remapped = static_cast<uint16_t>(0x8000 | ((lutValue & 0x06) << 12) | (lutValue & 0x01));

        if(lutValue == 0) {
            value = static_cast<uint8_t>((value & 0xC0) | LUT_REG[m_exRegs[2] & 0x07][value & 0x07]);
        }

        Mapper004::writePrg(remapped - 0x8000, value);
    }
