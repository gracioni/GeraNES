...
class Mapper059 : public BaseMapper
{
private:
    bool m_returnDipSwitch = false;
    uint8_t m_prgBank = 0;
    bool m_prg32Mode = false;
    uint8_t m_chrBank = 0;
    bool m_horizontalMirroring = false;
    uint8_t m_prg16Mask = 0;
    uint8_t m_prg32Mask = 0;
    uint8_t m_chrMask = 0;

public:
    Mapper059(ICartridgeData& cd) : BaseMapper(cd)
    {
        m_prg16Mask = calculateMask(cd.numberOfPRGBanks<BankSize::B16K>());
        m_prg32Mask = calculateMask(cd.numberOfPRGBanks<BankSize::B32K>());
    ...
    GERANES_HOT void writePrg(int addr, uint8_t /*data*/) override
    {
        ...
        m_prgBank = static_cast<uint8_t>((absolute & 0x70) >> 4);
        m_prg32Mode = (absolute & 0x80) == 0;
        m_chrBank = static_cast<uint8_t>(absolute & 0x07);
        m_horizontalMirroring = (absolute & 0x08) != 0;
        m_returnDipSwitch = (absolute & 0x100) != 0;
    }

    GERANES_HOT uint8_t readPrg(int addr) override
    {
        if(m_prg32Mode) {
            return cd().readPrg<BankSize::B32K>(static_cast<uint8_t>((m_prgBank >> 1) & m_prg32Mask), addr);
        }
        return cd().readPrg<BankSize::B16K>(m_prgBank & m_prg16Mask, addr);
    }
    ...
    {
        return m_returnDipSwitch ? 0 : openBusData;
    }
    ...
    void serialization(SerializationBase& s) override
    {
        ...
        SERIALIZEDATA(s, m_returnDipSwitch);
