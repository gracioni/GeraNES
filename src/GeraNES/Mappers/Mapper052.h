...
class Mapper052 : public Mapper004
{
private:
    uint8_t m_extraReg = 0;

    GERANES_INLINE uint8_t mapPrgPage(uint8_t page8k) const
    {
        if(m_extraReg & 0x08) {
            return static_cast<uint8_t>((page8k & 0x0F) | ((m_extraReg & 0x07) << 4)) & m_prgMask;
        }
        return static_cast<uint8_t>((page8k & 0x1F) | ((m_extraReg & 0x06) << 4)) & m_prgMask;
    }

    GERANES_INLINE uint16_t mapChrPage(uint16_t page1k) const
    {
        if(m_extraReg & 0x40) {
            return static_cast<uint16_t>((page1k & 0x7F) | (((m_extraReg & 0x04) | ((m_extraReg >> 4) & 0x03)) << 7)) & m_chrMask;
        }
        return static_cast<uint16_t>((page1k & 0xFF) | (((m_extraReg & 0x04) | ((m_extraReg >> 4) & 0x02)) << 7)) & m_chrMask;
    }
...
public:
    Mapper052(ICartridgeData& cd) : Mapper004(cd)
    {
    }

    GERANES_HOT void writeSaveRam(int addr, uint8_t data) override
    {
        if((m_extraReg & 0x80) == 0) {
            m_extraReg = data;
        } else {
