...
public:
    Mapper174(ICartridgeData& cd) : BaseMapper(cd)
    {
    ...
    GERANES_HOT void writePrg(int addr, uint8_t /*data*/) override
    {
        ...
        m_horizontalMirroring = (absolute & 0x0001) != 0;
        m_chrBank = static_cast<uint8_t>((absolute >> 1) & 0x07);
        m_prgBank = static_cast<uint8_t>((absolute >> 4) & 0x07);
        m_prg32Mode = (absolute & 0x0080) != 0;
    }
