...
class Mapper146 : public BaseMapper
{
private:
    uint8_t m_prgBank = 0;
    uint8_t m_chrBank = 0;
...
public:
    Mapper146(ICartridgeData& cd) : BaseMapper(cd)
    {
    }
    ...
    {
        if((addr & 0x0100) != 0) {
            m_prgBank = static_cast<uint8_t>((value >> 3) & 0x01);
            m_chrBank = static_cast<uint8_t>(value & 0x07);
        }
    }
