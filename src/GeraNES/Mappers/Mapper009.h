...
public:
    ...
    {
        numberOfPRG8kBanks  = m_cd.numberOfPRGBanks<WindowSize::W8K>();
    ...
    GERANES_HOT uint8_t readPrg(int addr) override
    {
        ...
        if(addr < 0x2000) return m_cd.readPrg<WindowSize::W8K>(m_PRGReg,addr);
        else if(addr < 0x4000) return m_cd.readPrg<WindowSize::W8K>(numberOfPRG8kBanks-3,addr);
        else if(addr < 0x6000) return m_cd.readPrg<WindowSize::W8K>(numberOfPRG8kBanks-2,addr);

        return m_cd.readPrg<WindowSize::W8K>(numberOfPRG8kBanks-1,addr);
    }
    ...
    GERANES_HOT uint8_t readChr(int addr) override
    {
        ...
        if(addr < 0x1000) {
            if(!m_latch1) ret = m_cd.readChr<WindowSize::W4K>(m_CHRReg0A, addr);
            else ret = m_cd.readChr<WindowSize::W4K>(m_CHRReg0B, addr);
        }
        else {
            if(!m_latch2) ret = m_cd.readChr<WindowSize::W4K>(m_CHRReg1A, addr);
            else ret = m_cd.readChr<WindowSize::W4K>(m_CHRReg1B, addr);
        }
