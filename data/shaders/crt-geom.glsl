...
{
    float colorLum = length(textureColor) * 0.5775;

    vec3 luminanceWeighting = vec3(0.3,0.6,0.1);
    if(colorLum < 0.5) luminanceWeighting.rgb=(luminanceWeighting.rgb*luminanceWeighting.rgb)+(luminanceWeighting.rgb*luminanceWeighting.rgb);
